def checkio(teleports_string):
    ...
    routes = [item for item in teleports_string.split(',')]
    start_p = end_p = '1'
    visit = ''
    result = find_route(routes, visit, start_p, end_p)
    return result

def find_route(routes, visit, start_p, end_p):
    for item in routes:
        if start_p in item:
            new_list = list(routes)
            new_list.remove(item)
            new_visit = visit + start_p
            p = find_route(new_list, new_visit, item.replace(start_p, "") ,end_p)
            if p:
                return start_p + p
    if len(set(visit)) >= 8 and start_p == end_p:
        return end_p
    return ''
...
if __name__ == "__main__":
    ...
    assert check_solution(checkio, "12,15,16,23,24,28,83,85,86,87,71,74,56"), "Second"
